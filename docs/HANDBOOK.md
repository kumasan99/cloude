...
最終更新: 2026-08-28 21:20 JST（トリガー26本を実査し体制表を更新／会議体3規程を納品）
...
停止中（enabled=false・掃除候補）: リン旧版2本 `trig_01RgM3TZ…` `trig_01Tsac6H…`／バタコ臨時 `trig_01JUKk71…`／Gmail追加ぶん `trig_01GRkP8v…`

※ リン・バタコ・ユカの「毎時30分 連絡板の指示確認」は**名前と実態が不一致**（cron `30 */3 * * *` ＝3時間おき）。名称の修正が必要。
