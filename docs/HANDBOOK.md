...
最終更新: 2026-08-29 01:40 JST（案A整流を実施／モデル方針＝デフォルトOpusを記録／メガホの定例なしは意図的と訂正）
...
- 背景: クロ・コダマ・セバスは configured=fable-5 だったが実行は opus-5 だった。原因は**フォールバックによる自動切り替え**（CEO説明）。基準をOpusに置けば、意図しない切り替わりが起きない
- **重要: トリガーの `model` は、既存セッションに発火するトリガーには効かない。**そのセッション自身のモデル設定が優先される。変更は各セッションで `/model` を実行する必要がある（社長操作）
- 毎回新規セッションを作る方式のジョブ（リン・バタコ・ユカ・チサト・Gmail）は既に opus-5 指定済み
- 切替が必要な3セッション: クロ `…5aG9ZFP`／コダマ `…n3r82ZJu`／セバス `…BdA9aMiP`
...
| 件 | 判断すること |
|---|---|
| モデル切替（3セッション） | 社長が各セッションで `/model` を実行。クロ・コダマ・セバス |
