...
`inspect` は★が埋まっていない前提のコマンドなので、**何も埋めずにそのまま**
実行して構いません。
...
たどれない段階があってもそこで止まらず、その時点で開いている画面を保存して
先に進みます。保存されなかった段階は「まだたどれていない段階」として、
どの★を埋めればよいかが最後に表示されます。

`out/inspect/` に保存されるもの:

| ファイル | 何を探すか |
| --- | --- |
| `00_ログイン直後.html` | `home_url` の画面。事業所一覧・利用者一覧への入口 |
| `01_事業所一覧.html` | `offices.*` のセレクタ |
| `02_利用者一覧.html` | `users.*` のセレクタ |
| `03_提供実績記録表.html` / `.png` | `record.*` のセレクタ |

**★を少し埋める → `inspect` を実行し直す**、を繰り返して1段ずつ先に進めるのが
確実です。★が残ったまま `run` を実行した場合は、埋めるべき項目名
（例: `config.json の offices.list_url がまだ★のままです`）を表示して止まります。
...
| 項目 | 説明 |
| --- | --- |
| `home_url` | ログイン後に最初に開く画面。`inspect` の起点になる |
...
- 氏名の連続スペース（「岩村  伸一」）がファイル名に保たれること
- ★が未設定でも `inspect` が画面を保存できること
- ★が未設定のまま `run` すると、埋めるべき項目を示して止まること
- 2回目の実行で既存分がスキップされること
